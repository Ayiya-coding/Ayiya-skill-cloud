...
## Field Notes (2026-04 Incident Hardening)

These checks are based on real deployment failures and should be treated as mandatory for split frontend/backend Vercel deployments.

### 1) White screen after deploy (frontend is up, app is blank)

Symptoms:
- Browser console shows `Minified React error #31`
- Multiple `404` on `/<frontend-domain>/api/*`

Root causes:
- Frontend used relative API base (`/api`) but backend was deployed on a different Vercel project/domain
- Error payload objects (for example `{ code, message }`) were rendered directly as React children

Fix:
- Use explicit API base for production split deployments:
  - Prefer `VITE_API_BASE_URL`
  - If absent and hostname ends with `.vercel.app`, default to backend production URL
  - Keep local fallback `/api` for same-origin dev
- Normalize all UI-facing errors to strings before rendering (`String(...)` or object-safe formatter)
- Build SSE URLs from API base (do not hardcode `'/api/events/stream'` when frontend/backend are split)

Prevention checklist:
- Verify frontend bundle contains expected API base logic before promotion
- Smoke test: open page, confirm no React runtime invariant errors
- Smoke test: key API calls from frontend resolve to backend domain, not frontend domain

### 2) Deployment appears healthy but selected API endpoints return 404

How to distinguish quickly:
- Vercel edge 404: response body includes `NOT_FOUND` and headers contain `x-vercel-error: NOT_FOUND`
- Express route 404: response body typically `Cannot GET /...` with `x-powered-by: Express`

Fix workflow:
1. Verify route exists in backend router registration (`app.use('/api/...', ...)`)
2. Verify serverless entrypoint exists and exports app (`api/[...path].js` -> `export default app`)
3. If catch-all does not map as expected in your framework setup, add explicit `vercel.json` rewrites for `/api/:path*`
4. Redeploy and re-check target routes directly

Prevention checklist:
- Post-deploy route probe for critical paths (`/api/health`, business-critical `/api/*`)
- Include both unauthenticated and authenticated probes (401 can be expected; edge 404 is not)

### 3) Vercel auth/protection blocks debugging (401 while fetching deployment URL)

Symptoms:
- Direct fetch to deployment URL returns Vercel auth page / 401

Fix:
- For protected deployments, use:
  - `get_access_to_vercel_url` to create temporary share URL
  - `web_fetch_vercel_url` to fetch through Vercel-authenticated context

Prevention checklist:
- Do not conclude endpoint failures from unauthenticated fetches on protected deployments
- Always establish authenticated fetch path first

### 4) Local CLI/build failures on Windows automation environments

Observed failures and remedies:
- `No project settings found locally` -> run `vercel pull --yes --environment <env>` or `vercel build --yes`
- `specified token is not valid` -> re-auth (`vercel login`) or refresh `VERCEL_TOKEN`
- `spawn EPERM` / cannot create `AppData\\Roaming\\com.vercel.cli` -> run outside restricted sandbox / ensure writable user profile
- `npm`/`npx` not in PATH -> invoke CLI via explicit Node runtime and `vercel/dist/index.js`, or install Node/NPM correctly

Prevention checklist:
- CI/local automation must verify:
  - valid token
  - linked project settings (`.vercel/project.json`)
  - writable CLI state directory
  - available Node+NPM runtime

### 5) Split-project production defaults (recommended baseline)

- Frontend:
  - `VITE_API_BASE_URL=https://<backend-production-domain>/api`
  - Error normalization before rendering
  - SSE endpoint built from API base
- Backend:
  - Stable `/api/health`
  - Catch-all API handler verified on deployed URL

### 6) Release gate (must pass before marking deployment done)

1. Confirm alias points to latest production deployment (`vercel inspect` / deployment metadata)
2. Confirm frontend HTML references latest JS/CSS assets
3. Confirm backend `/api/health` returns 200
4. Confirm critical `/api/*` routes do not return edge `NOT_FOUND`
5. Confirm frontend loads with no React invariant runtime crash
